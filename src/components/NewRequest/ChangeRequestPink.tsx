...
const ChangeRequestPink: React.FC = () => {
    const datasets = [
        ...
        const fetchClients = async () => {
            ...
            try {
                const response = await axios.get(
                    `${process.env.NEXT_PUBLIC_API_URL}/api/request_get_pink`,
                    {
                        params: {
                            userId,
                            requestId
                ...
                setCurrentRequest(response.data.requestPink);
                let tp_checkedCategories = {};
                let tp_checkedItems = {};
                if (response.data.requestPink.areaSelection) {
                    const areaConditionData = transformData(
                        response.data.requestPink.areaSelection, // Corrected from mainCondition to areaSelection
    ...
    const confirmValues = () => {
        ...
        if(requestData.projectName === "" || (requestData.wishNum ?? -1) < 0 || Object.keys(requestData.areaSelection).length === 0){
            alert("必須項目を入力してください。");
            return 0;
        }
...
export default ChangeRequestPink;